...
- **Preparação:** fiz um novo login com GitHub e confirmei que a página exibia a sessão ativa. Em seguida, abri o console do banco D1 `oauth-sessions-frank`.
- **Pedido enviado:** executei no console `UPDATE sessions SET expires_at = 0;`. Depois, digitei o endereço `/api/me` diretamente na barra de endereço de uma aba nova, para que o navegador enviasse o cookie `SameSite=Strict`, e consultei `GET /api/me`.
- **Resultado esperado:** `/api/me` responde 401.
- **Resultado observado:** o console D1 informou `This query successfully executed.`. Em seguida, `GET /api/me` respondeu `401 Unauthorized` com o corpo `{"error":"sessao invalida"}` e `Cache-Control: no-store`. Essa mensagem indica que o cookie chegou ao servidor e a sessão foi recusada por estar expirada.
- **Observação:** em uma primeira execução, a resposta foi `{"error":"sem sessao"}`, mensagem que indica que o servidor não recebeu cookie. Isso provavelmente ocorreu porque a página foi aberta a partir de um link de outro site, situação em que o navegador não envia cookies `SameSite=Strict`. O teste foi então repetido com o endereço digitado, e o resultado acima é o da repetição.
